...
* Victor Getmanskiy presented the high level overview of oneIPL. oneIPL provides the DPC++ API for image processing functionality, which is inherited from classic Intel IPP and is supporting XPU execution. So all oneIPL DPC++ API contains CPU optimized kernels, kernels optimized for GPU and could be also extended to support other xPU optimisations in future.
...
* Currently the provisional oneIPL Spec v0.5 is published. It contains all functionality which is targeted for oneIPL 2022 Q1 beta release. The functionality contains most commonly used functions like Geometry transformations (Resizing, mirroring), Color conversions (RGB to Plane, RGB to NV12), Filtering, Type conversion and other functions.
